...
import webhook.exception.HttpException;
import webhook.receive.EntityFactory;
import webhook.receive.ReadonlyMessage;
import webhook.send.AllowedMentions;
import webhook.send.WebhookEmbed;
import webhook.send.WebhookMessage;
import webhook.send.WebhookMessageBuilder;
import webhook.util.ThreadPools;
import webhook.util.WebhookErrorHandler;
...
public class WebhookClient implements AutoCloseable {
     ...
     * specified by {@link webhook.WebhookClientBuilder#WebhookClientBuilder(String)} explicitly
     *
    ...
    /**
     * Whether futures will receive {@link webhook.receive.ReadonlyMessage} instances
     * or {@code null}.
     *
    ...
    public WebhookClient setTimeout(long millis) {
        if (millis < 0)
    ...
    /**
     * Sends the provided {@link webhook.send.WebhookMessage}
     * to the webhook.
     * <br>The returned future receives {@code null} if {@link webhook.WebhookClientBuilder#setWait(boolean)}
     * was set to false.
     *
     ...
     *
     * @see    #isWait()
     * @see    #send(webhook.send.WebhookMessage)
     */
    @NotNull
    ...
    /**
     * Sends the provided {@link webhook.send.WebhookEmbed} to the webhook.
     * <br>The returned future receives {@code null} if {@link webhook.WebhookClientBuilder#setWait(boolean)}
     * was set to false.
     *
     * @param  first
    ...
    /**
     * Edits the target message and updates it with the provided {@link webhook.send.WebhookMessage}
     * to the webhook.
     * <br>The returned future receives {@code null} if {@link webhook.WebhookClientBuilder#setWait(boolean)}
     * was set to false.
     *
    ...
    /**
     * Edits the target message and updates it with the provided {@link webhook.send.WebhookEmbed} to the webhook.
     * <br>The returned future receives {@code null} if {@link webhook.WebhookClientBuilder#setWait(boolean)}
     * was set to false.
     *
     * @param  messageId
     ...
     *
     * @see    #isWait()
     * @see    #edit(long, webhook.send.WebhookMessage)
     */
    @NotNull
    ...
    private boolean enqueuePair(Request pair) {
